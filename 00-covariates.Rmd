...
saveRDS(ids_final, file = here::here("data", base, "ids_final"))
```
...
Lastly, scale all variables EXCEPT par, which has already been scaled. The
scaling will be done one variable at a time, by pooling all measurements from
all cruises and applying a common scaling.
...
```{sh}
scp -r ~/repos/flowmixapp/data/00-covariates/*-covariates-common-scaled.RDS sangwonh@discovery.usc.edu:~/repos/flowmixapp/data/00-covariates/.
```
