...
```{sh, eval = FALSE}
## Uploading directory.
scp -r /media/sangwonh/Backup\ Plus/flowmixapp/data/01-cytograms/SCOPE_3 sangwonh@discovery.usc.edu:repos/flowmixapp/data/01-cytograms/.
scp -r /media/sangwonh/Backup\ Plus/flowmixapp/data/01-cytograms/SCOPE_5 sangwonh@discovery.usc.edu:repos/flowmixapp/data/01-cytograms/.
scp -r /media/sangwonh/Backup\ Plus/flowmixapp/data/01-cytograms/SCOPE_6 sangwonh@discovery.usc.edu:repos/flowmixapp/data/01-cytograms/.
scp -r /media/sangwonh/Backup\ Plus/flowmixapp/data/01-cytograms/SCOPE_10 sangwonh@discovery.usc.edu:repos/flowmixapp/data/01-cytograms/.
scp -r /media/sangwonh/Backup\ Plus/flowmixapp/data/01-cytograms/SCOPE_11 sangwonh@discovery.usc.edu:repos/flowmixapp/data/01-cytograms/.
scp -r /media/sangwonh/Backup\ Plus/flowmixapp/data/01-cytograms/SCOPE_19 sangwonh@discovery.usc.edu:repos/flowmixapp/data/01-cytograms/.
```
...
3. Run the code in `~/scripts/flowmixapp/bin.R`.
* This uses helpers from `~/scripts/flowmixapp/01-helpers.R`

4. This saves to an RDS file (e.g. `KM1508-y.RDS`), on the server. Download this
   file from the server to `datadir = "~/repos/flowmixapp/data/01-cytograms/."`,
   using this script:
...
```
This data is further rescaled so that the diameter is between 0 and 8.

## Make the series of plots

We'll load and plot this data, in several ways. First, 1d plots:

```{r 1d-plots, fig.width = 10, fig.height = 3}
datadir = "~/repos/flowmixapp/data"##/01-cytograms"

all_cruise_id = c("KM1508", "KM1512", "KOK1515", "KM1518", "KOK1609") ## KM1513 doesn't bin correctly, for now.
for(cruise_id in all_cruise_id){
  ## print(cruise_id)

  ## Load the binned 3d data
  ## cruise_id = "KOK1609" ##"KOK1515" ##"KM1512"
  ...
  ##  Check that the data has the same dates as X.
  filename = paste0(cruise_id, "-covariates-scaled.RDS")
  X = readRDS(file = here::here("data", "00-covariates", filename))
  twotimes = tibble(Xtime = X$time, ytime = ylist %>% names() %>% lubridate::as_datetime())
  stopifnot(all(twotimes$Xtime == twotimes$ytime))
  ...
  for(dimname in c("diam", "chl", "pe")){
    ...
    p = p + ggtitle(paste0(cruise_id, " - ", dimname))
    p = p + theme_minimal()
    plot(p)
...
## cruise_id = "KM1508"##"KOK1515"##"KOK1609"##"KM1512" ##"KOK1515" ##"KM1512"
for(cruise_id in all_cruise_id){
  filename = paste0(cruise_id, "-datobj.RDS")
  ybin_obj = readRDS(file = file.path(datadir, "01-cytograms", cruise_id, "data", filename))
  ylist = ybin_obj$ylist
