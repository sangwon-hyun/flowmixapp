...
* Size of time blocks: blocks of 5 hours? 
  ...
  + Then, CV is done by using blocks {1, 6, 11, ..} as fold 1, {2, 7, 12, ..} as
    fold 2.
* Radius for restricting the cluster means over time: 0.5 or 1? Let's start with
  1 (for flexibility). Seems to work well.

# Notes on computation (from pilot)

* For KM1508, one iteration costs 2 or 2.5 seconds per iteration.
* On 500 cores, entire CV can be done in 15-30 minutes.
* Subsampling will take (100 x 30 min = 50 hrs = 1 day).
...
# do
#   from=sangwonh@discovery.usc.edu:scratchdir/output/flowmixapp/data/02-estimate/$cruise_id/output/
#   to=/home/sangwonh/repos/flowmixapp/data/02-estimate/$cruise_id/output/orig/.
#   mkdir -p  $to
#   rsync -auv $from $to
# done

# for cruise_id in KM1508 KM1512 KM1513 KOK1515 KM1518 KM1601 KM1602 KOK1604 KOK1606 KOK1607 KOK1608 KOK1609  
# do
#     from=~/repos/flowmixapp/data/02-estimate/$cruise_id/summaries/summary-orig.RDS
#       to=~/repos/flowmixapp/data/02-estimate/$cruise_id/summaries/yes-wind-twelve-clusters/.
# 	mkdir -p $to
# 	mv $from $to
# done

  for cruise_id in KM1508 KM1512 KM1513 KOK1515 KM1518 KM1601 KM1602 KOK1604 KOK1606 KOK1607 KOK1608 KOK1609 MGL1704 KM1708 KM1709 KM1713 KM1717  KOK1801 KM1802 KOK1803 FK180310-1 FK180310-2
do
  echo $cruise_id
  from=sangwonh@discovery.usc.edu:scratchdir/output/flowmixapp/data/02-estimate/$cruise_id/summaries/*.RDS
...
all_cruise_id = c(c("KM1508", "KM1512", "KM1513",
                  ...
                  c("KOK1607", "KOK1608", "KOK1609"), ##"KOK1608a", , "KOK1609a"
                  ## c("KM1712"), ## Need to download covariates, bin properly and run again.
                  c("MGL1704", "KM1708", "KM1709", "KM1713", "KM1717", "KOK1801", "KM1802"), ## These are new.
                  c("KOK1803",  "FK180310-1", "FK180310-2")) ## "KM1805" ## Too little data in 1805
...
for(cruise_id in all_cruise_id){
  ...
  for(tt in 1:TT){
    ...
    ## plotfile = file.path(file.path(outputdir, cruise_id, "viz", "unlabeled"), filename) ## Temporary
...
Also, visualize the coefficients.
...
Diagnostics, because you can never be too careful.. 

* The CV score matrix of the original data run (shown as a heatmap).
* How sparse are these models (shown as tables)? 

```{r diagnostics, fig.width=5, fig.height=5, results='asis'}
for(cruise_id in all_cruise_id){
  cat('###', cruise_id,' \n')
  ...
  kable(alphadf, caption = "Number of nonzero alpha coeffs") %>% print() ## title "alpha"
  kable(betadf, caption = "Number of nonzero beta coeffs")  %>% print()## title "beta"
  cat('\n\n')
}
```
...
```{sh, eval=FALSE}
for cruise_id in KM1508 KM1512 KM1513 KOK1515 KM1518 KM1601 KM1602 KOK1604 KOK1606 KOK1607 KOK1608 KOK1609 MGL1704 KM1708  KM1712 KM1713 KM1717  KOK1801 KM1802 KOK1803 FK180310-1 FK180310-2
do
	echo $cruise_id
	from=sangwonh@discovery.usc.edu:scratchdir/output/flowmixapp/data/02-estimate/$cruise_id/summaries/*
	to=/home/sangwonh/repos/flowmixapp/data/02-estimate/$cruise_id/summaries/.
	mkdir -p  $to
	rsync -auv $from $to
done

## Move summaries to "no-wind" folder
for cruise_id in KM1508 KM1512 KM1513 KOK1515 KM1518 KM1601 KM1602 KOK1604 KOK1606 KOK1607 KOK1608 KOK1609;
do
	from=/home/sangwonh/repos/flowmixapp/data/02-estimate/$cruise_id/summaries/*.RDS
	  to=/home/sangwonh/repos/flowmixapp/data/02-estimate/$cruise_id/summaries/yes-wind-twelve-clusters/.
	mkdir -p  $to
	mv $from $to
done
...
for cruise_id in KM1508 KM1512 KM1513 KOK1515 KM1518 KM1601 KM1602 KOK1604 KOK1606 KOK1607 KOK1608 KOK1609
do
  rm $cruise_id/summaries/*
  from=$cruise_id/output/*
  to=$cruise_id/output/old
  mkdir -p $to
  mv $from $to
done
```
...
-- original *and* subsampling replicates. This creates summary files named e.g.
`KM1601-grand-summary.RDS`. 
...
for(cruise_id in all_cruise_id){
  ...
  subsample_summarize(cruise_id = cruise_id,
                      save = TRUE,
                      nsim = 100,
                      outputdir = "~/scratchdir/output/flowmixapp/data/02-estimate",
                      outputdir_cytograms = "~/scratchdir/output/flowmixapp/data/01-cytograms",
                      verbose = TRUE)
  }, error = function(e){
...
for(cruise_id in all_cruise_id){
  ...
  for(iclust in 1:numclust){
    ...
    threetables %>% do.call(cbind, .) %>% kable(caption="beta coefficients") %>% print()
      cat(fill = TRUE)
  }
}
```
...
First gather all the subsampling summaries.
...
pred_long = pred_long %>%  select(cruise_id, time, population, type, value) 
...
**Data** It's possible to make a long matrix with binned data that includes the columns.
...
Plotting the total biomasses.

```{r}
dat_long = readRDS(file.path(outputdir0, "dat_long_main.RDS"))
dat_long_sum = dat_long %>% group_by(cruise_id, time) %>% summarise(sumcounts = sum(counts))
dat_long_sum %>% ggplot() +
  facet_wrap(~cruise_id, scale = "free_x") +
  geom_line(aes(x=time, y=sumcounts), alpha=.5)+
  geom_point(aes(x=time, y=sumcounts), size=rel(.5)) +
  ylab("Total biomass")
```
...
```{r}
X_long = readRDS(file = file.path(outputdir, "X_long.RDS"))
pred_long = readRDS(file = file.path(outputdir, "pred_long.RDS"))
## pred_long = readRDS(file = file.path(outputdir, "pred_long_no_wind.RDS"))
coef_long = readRDS(file = file.path(outputdir, "coef_long.RDS"))
print(X_long)
print(pred_long)
print(coef_long)
```

```{r example_X, fig.width=20, fig.height=20}
X_long  %>% ggplot() +
  facet_grid(variable~cruise_id, scale = "free", space = "free") +
  geom_line(aes(x=time, y=value)) +
  theme(strip.text.y = element_text(angle = 0))
```


```{r example_pred, fig.width=20, fig.height=10}
pred_long  %>%
  dplyr::filter(type %in% c("mu_diam", "prob")) %>%
  pivot_wider(names_from = "type", values_from = "value") %>% 
  ggplot() +## +
  facet_grid(population~cruise_id, scale = "free_x", space = "free") +
  geom_line(aes(x=time, y=mu_diam), lwd = .5, col = rgb(0,0,0,0.2))  +
  geom_point(aes(x=time, y=mu_diam, size = prob), pch=16, col = rgb(0,0,0,0.8)) + 
...
```{r example_coef, fig.width=20, fig.height=20}
coef_long %>%
  dplyr::filter(variable != "intp") %>%
  ## dplyr::filter(type == "alpha") %>%
  dplyr::filter(grepl("beta",type)) %>% 
  ggplot() +
  facet_grid(type+variable~population) +
  geom_boxplot(aes(stab, col = type), pch = "|", alpha = .5) +
  xlab(c(0,1)) +
  scale_y_discrete(breaks=NULL) + ylab("") +
  ## theme(panel.background = element_rect(fill = NA, color = "black")) +
  theme(strip.text.y = element_text(angle = 0)) +
  theme(panel.spacing = unit(0.1, "lines")) +
  ggtitle(paste0("beta_diam"))
```
...
## Read in the summary over all subsampled models.
cruise_id = "KM1508"
summarydir = "~/repos/flowmixapp/data/02-estimate/KM1508/summaries/"
filename = paste0(cruise_id, "-grand-summary.RDS")
obj = readRDS(file = file.path(summarydir, filename))
names(obj)
obj$stat_list %>% length()
iclust = 1
idim = 1
obj$stat_list %>% .[[iclust]] %>% [,idim]
```
